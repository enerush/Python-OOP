...
Вам требуется сформировать класс PathLines для описания маршрутов, состоящих из линейных сегментов. При этом каждый
линейный сегмент предполагается задавать отдельным классом LineTo. Объекты этого класса будут формироваться командой:
line = LineTo(x, y)
...
class PathLines:
    ...
    def __init__(self, *args):
        self.lst = [LineTo(0, 0)] + list(args)

    def get_path(self):
        return self.lst[1:]

    def get_length(self):
        g = ((self.lst[i - 1], self.lst[i]) for i in range(1, len(self.lst)))
