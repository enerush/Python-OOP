...
class LessonItem:
    ...
    def __setattr__(self, key, value):
        if (key == 'title' and type(value) is not str) or (key in ('practices', 'duration') and\
            type(value) is not int) or (key in ('practices', 'duration') and type(value) is int and value < 0):
            raise TypeError("Неверный тип присваиваемых данных.")
        else:
