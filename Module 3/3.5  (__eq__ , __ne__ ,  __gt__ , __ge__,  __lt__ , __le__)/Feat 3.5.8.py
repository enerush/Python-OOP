...
class MoneyR:
    ...
    def __eq__(self, other):
        self._check_cb(other)
        return other.in_RUB - 0.1 <= self.in_RUB <= other.in_RUB + 0.1

    def __gt__(self, other):
        self._check_cb(other)
        return self.in_RUB > other.in_RUB + 0.1
...
class CentralBank:
    ...
    def register(cls, money):
        money.cb = cls
        if type(money) is MoneyE:
            money.in_RUB = money.volume * money.cb.rates['euro'] * money.cb.rates['rub']
        if type(money) is MoneyD:
            money.in_RUB = money.volume * money.cb.rates['rub']
        if type(money) is MoneyR:
            money.in_RUB = money.volume
