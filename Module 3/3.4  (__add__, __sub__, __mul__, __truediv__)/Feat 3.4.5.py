...
class ListMath:
    ...
    def __init__(self, lst=None):
        self.lst_math = self._clean_lst(lst) if lst and type(lst) is list else []
    ...
    def __add__(self, other):
        if type(other) not in (int, float):
            raise ArithmeticError('Right perand`s wrong type')
    ...
    def __truediv__(self, other):
        if type(other) not in (int, float):
            raise ArithmeticError('Right operand`s wrong type')
        return ListMath([i / other for i in self.lst_math])
