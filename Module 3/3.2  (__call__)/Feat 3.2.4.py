"""
Подвиг 4.
Предположим, мы разрабатываем класс для обработки формы авторизации на стороне сервера. Для этого был создан следующий
класс с именем LoginForm:
...
class LengthValidator:
    def __init__(self, min_len, max_len):
        self._min__len = min_len
        self._max__len = max_len

    def __call__(self, string, *args, **kwargs):
        if self._min__len <= len(string) <= self._max__len:
            return True
        return False
...
class CharsValidator:
    ...
    def __call__(self, chars, *args, **kwargs):
        Flag = True
        for c in chars:
